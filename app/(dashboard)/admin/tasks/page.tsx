...
import { Loader2, Kanban, List, LayoutGrid, AlertCircle, Users, Check, X, Zap, Search, Filter, ChevronDown, FolderSearch, Clock, Tag } from "lucide-react";
...
export default function AdminTasksPage() {
  ...
  return (
    <main className="space-y-8 p-4 md:p-8 animate-in fade-in duration-700" aria-label="Task Management Dashboard">
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-1 text-center md:text-left">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-zinc-900 dark:text-zinc-100 italic">Deployment Strategy</h1>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-9 rounded-xl border-zinc-200 dark:border-zinc-800 gap-2 font-bold px-3">
                ...
                <span className="capitalize">{statusFilter === 'all' ? 'Any Status' : statusFilter.replace('_', ' ')}</span>
                <ChevronDown className="h-3.5 w-3.5 opacity-50" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded-xl w-40 p-1">
              <DropdownMenuLabel className="text-[10px] uppercase tracking-widest opacity-50 px-2 py-1.5">Status Filter</DropdownMenuLabel>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-9 rounded-xl border-zinc-200 dark:border-zinc-800 gap-2 font-bold px-3">
                ...
                <span className="capitalize">{priorityFilter === 'all' ? 'Any Priority' : priorityFilter}</span>
                <ChevronDown className="h-3.5 w-3.5 opacity-50" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded-xl w-40 p-1">
              <DropdownMenuLabel className="text-[10px] uppercase tracking-widest opacity-50 px-2 py-1.5">Priority Filter</DropdownMenuLabel>
                ...
                <span>{
                  assigneeFilter === 'all' ? 'Any Assignee' :
                    assigneeFilter === 'unassigned' ? 'Unassigned' :
                      employees.find(e => e.id === assigneeFilter)?.full_name?.split(' ')[0] || 'Assignee'
                }</span>
                <ChevronDown className="h-3.5 w-3.5 opacity-50" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="rounded-xl w-56 p-1 max-h-80 overflow-y-auto mt-1 shadow-2xl">
              <DropdownMenuLabel className="text-[10px] uppercase tracking-widest opacity-50 px-2 py-1.5 font-black">Members Filter</DropdownMenuLabel>
      ...
      <Tabs defaultValue="list" className="space-y-6">
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              {task.projects && (
                                <Badge variant="outline" className="text-[8px] font-black uppercase text-blue-500 border-blue-100 bg-blue-50/30 px-1 py-0 h-4 tracking-tighter shrink-0 ring-1 ring-blue-500/10">
                                  <FolderSearch className="h-2 w-2 mr-0.5" />
                                  {task.projects.name}
                                </Badge>
                              )}
                          ...
                          {task.assignment_status === 'pending_approval' && (
                            <div className="flex items-center gap-1">
                              <Button
                                size="icon"
                                variant="ghost"
                                className="h-7 w-7 rounded-full bg-green-50 text-green-600 border border-green-100"
                                onClick={() => handleApproveClaim(task.id)}
                              ...
                              <Button
                                size="icon"
                                variant="ghost"
                                className="h-7 w-7 rounded-full bg-red-50 text-red-600 border border-red-100"
                                onClick={() => handleRejectClaim(task.id)}
                                disabled={processingId === task.id}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                      ...
                      {isLoading ? (
                          ...
                          >
                              ...
                              <div className="flex items-start gap-2">
                                <div className="flex-1 min-w-0">
                                  ...
                                  {task.is_open_assignment && (
                                    <Badge variant="outline" className="text-[9px] mt-1.5 font-black text-blue-600 border-blue-100 bg-blue-50/50 flex items-center gap-1 w-fit ring-1 ring-blue-500/10">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="relative">
                                  <Avatar className="h-8 w-8 border-2 border-white dark:border-zinc-800 shadow-sm ring-1 ring-zinc-100 dark:ring-zinc-800">
                                    <AvatarImage src={task.profiles?.avatar_url || undefined} />
                                    <AvatarFallback className="text-[9px] font-black bg-muted">
                                      {task.profiles?.full_name?.charAt(0) || 'U'}
                                    </AvatarFallback>
                                  </Avatar>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex flex-col gap-1.5 items-start">
                                {getStatusBadge(task.status)}
                                <Badge variant="outline" className={`font-black text-[8px] uppercase px-1.5 h-4 tracking-tighter ${getPriorityColor(task.priority)}`}>
                                  {task.priority}
                                </Badge>
                              </div>
                            </td>
                              ...
                              <div className="flex justify-end gap-2">
                                {task.assignment_status === 'pending_approval' ? (
                                  <>
                                    <Button
                                      size="sm"
                                      className="h-8 rounded-xl font-black text-[10px] uppercase tracking-widest bg-green-600 hover:bg-green-700 text-white shadow-lg shadow-green-500/20"
                                      onClick={() => handleApproveClaim(task.id)}
                                      disabled={processingId === task.id}
                                    >
                                    ...
                                    <Button
                                      size="sm"
                                      variant="outline"
                                      className="h-8 rounded-xl font-black text-[10px] uppercase tracking-widest border-red-200 text-red-600 hover:bg-red-50"
                                      onClick={() => handleRejectClaim(task.id)}
                                      disabled={processingId === task.id}
                                    >
                        ...
                        <tr>
                          <td colSpan={6} className="px-6 py-32 text-center">
                            ...
                            <p className="text-sm text-zinc-500 font-medium italic mt-1 max-w-[200px] mx-auto leading-relaxed">Adjust your filters or search terms to find what you're looking for.</p>
                          </td>
                        </tr>
                      )}
                    </tbody>
                  ...
                  <Card className="border-none shadow-xl shadow-zinc-200/50 dark:shadow-none bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl group hover:shadow-2xl transition-all duration-500 rounded-[2rem] overflow-hidden">
                    <CardContent className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-lg font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-tight truncate leading-none mb-1">
                            {emp.full_name}
                          </p>
                          <div className="flex items-center gap-1.5">
                            <Badge variant="secondary" className="text-[9px] font-black uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 text-zinc-500 px-1.5 h-4">
                              {emp.role || "Employee"}
                            </Badge>
                      ...
                      <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-black text-zinc-900 dark:text-zinc-100">{emp.tasks?.length || 0}</span>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
        ...
        <TabsContent value="board">
          <div className="py-20 text-center opacity-40">
            ...
            <p className="text-lg font-bold uppercase tracking-widest">Kanban Board Under Development</p>
          </div>
        </TabsContent>
      </Tabs>
    </main>
