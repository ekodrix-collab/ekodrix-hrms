    ...
    <div className="space-y-8 p-4 md:p-8 animate-in fade-in duration-700">
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-1 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-2 text-purple-600 font-black text-xs uppercase tracking-widest">
            <Receipt className="h-3 w-3" />
            Financial Hub
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-zinc-900 dark:text-zinc-100">Expense Management</h1>
          <p className="text-muted-foreground font-medium text-sm md:text-base">Track and audit company expenditures efficiently.</p>
        </div>
        ...
        <div className="flex justify-center md:justify-end">
          <Card className="bg-purple-600 text-white border-none px-6 py-3 flex items-center gap-4 shadow-xl shadow-purple-500/20">
            ...
            <div>
              ...
              <p className="text-xl md:text-2xl font-black tabular-nums">₹{totalSpent.toLocaleString()}</p>
            </div>
          </Card>
        </div>
      </header>
                ...
                <div key={expense.id} className="p-4 space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="space-y-0.5">
                      <p className="text-sm font-bold uppercase text-zinc-900 dark:text-zinc-100">{expense.description}</p>
                      <p className="text-[10px] text-muted-foreground font-bold">{format(new Date(expense.expense_date), 'MMM dd, yyyy')}</p>
                    </div>
                    <span className="text-sm font-black tabular-nums text-purple-600">₹{Number(expense.amount).toLocaleString()}</span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-zinc-50 dark:border-zinc-800">
                    <span className="text-[9px] font-black uppercase text-zinc-400">Paid via: </span>
                    <span className="text-[9px] font-black uppercase text-zinc-900 dark:text-zinc-100">{expense.payment_method}</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="p-12 text-center opacity-40 font-bold text-sm">No expenses logged yet.</div>
            )}
          </div>
